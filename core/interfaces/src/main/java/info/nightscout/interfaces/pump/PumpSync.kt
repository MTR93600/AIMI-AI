...
interface PumpSync {
    ...
    data class PumpState(val temporaryBasal: TemporaryBasal?, val extendedBolus: ExtendedBolus?, val bolus: Bolus?, val profile: Profile?, val serialNumber: String) {
        ...
        data class TemporaryBasal @JvmOverloads constructor(
            ...
            val desiredRate: Double? = null,
            val desiredPct: Int? = null
        ) {
    ...
    */
    ...
    fun syncStopTemporaryBasalWithPumpIdMedLink(timestamp: Long, endPumpId: Long, pumpType: PumpType, pumpSerial: String): Boolean
    fun syncBolusWithTempIdMedLink(timestamp: Long, amount: Double, temporaryId: Long, type: DetailedBolusInfo.BolusType?, pumpId: Long?, pumpType: PumpType, pumpSerial: String): Boolean
}