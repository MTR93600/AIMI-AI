...
public class BgGraphBuilder {
    ...
    public List<Line> defaultLines() {
        ...
        lines.add(bolusLine((float) minChart));                                                                     // Bolus line shifted in front of Carb line
    ...
    private Line bolusLine(float offset) {
        ...
        for (BolusWatchData bwd: bolusWatchDataList) {
            if(bwd.date > start_time && bwd.date <= end_time && !bwd.isSMB && bwd.isValid && bwd.bolus > 0) {
                pointValues.add(new PointValue(fuzz(bwd.date), (float) offset-4*pointSize));                    // Down offset of bolus line increased
            }
        }
