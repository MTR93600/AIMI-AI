...
import info.nightscout.interfaces.plugin.*
...
open class MedLinkMedtronicPumpPlugin @Inject constructor(
    ...
    PluginDescription() //
        ...
        .pluginIcon(info.nightscout.core.ui.R.drawable.ic_veo_medlink)
    ...
    private fun stopPump(callback: Callback) {
        ...
        if (currentLevel - batteryDelta * 5 <= minimumBatteryLevel || currentLevel <= minimumBatteryLevel || pumpStatusData.deviceBatteryRemaining != 0 &&
            ...
            Thread {
                ...
                uiInteraction.runAlarm(rh.gs(R.string.medlink_medtronic_cmd_stop_could_not_be_delivered), rh.gs(R.string.medtronic_warning), info.nightscout.core.ui.R.raw.boluserror)
            }.start()
    ...
    override fun storeCancelTempBasal() {
        ...
        pumpSync.syncStopTemporaryBasalWithPumpIdMedLink(dateUtil.now(), dateUtil.now(), pumpType, medLinkPumpStatus.serialNumber)
        // val runningTBR = pumpStatusData.runningTBR
        // when {
        //     runningTBR != null -> {
        //         val differenceTime = System.currentTimeMillis() - runningTBR
        ...
        //                 "pumpSerial=${medLinkPumpStatus.serialNumber}] - Result: $result"
        //         )
        //     }
        // }
    ...
    private fun startPump(callback: Callback?) {
                ...
                PumpDriverState.Busy      -> {
                    ...
                    callback.invoke(
                        PumpEnactResult(injector) //
                            .success(false) //
                            .enacted(false) //
                            .comment(rh.gs(string.temp_basal_delivery_error))
                    )
                }
                ...
                PumpDriverState.Suspended -> {
                    ...
                    callback.invoke(
                        PumpEnactResult(injector) //
                            .success(!isConnect) //
                            .enacted(!isConnect) //
                            .comment(rh.gs(info.nightscout.core.ui.R.string.temp_basal_delivery_error))
                    )
                    if (!isConnect) {
                        lastStatus = PumpRunningState.Suspended.status
                        tempBasalMicrobolusOperations.operations.peek()
    ...
    fun createTemporaryBasalData(duration: Int, dose: Double) {
        val type: PumpSync.TemporaryBasalType = if (dose == 0.0) PumpSync.TemporaryBasalType.PUMP_SUSPEND else {
        ...
        if (timeDiff > 20) {
            if (clock.localDeviceTime.year <= 2015 || timeDiff <= 24 * 60 * 60) {
                ...
                if (clock.timeDifference == 0) {
                    uiInteraction.addNotificationValidFor(Notification.INSIGHT_DATE_TIME_UPDATED, rh.gs(string.pump_time_updated), Notification.INFO, 60)
                }
            } else {
    ...
    fun setTempBasalAbsolute(
        ...
        if (isPumpNotReachable) {
            ...
            return PumpEnactResult(injector) //
                .success(false) //
                .enacted(false) //
                .comment(rh.gs(string.pump_unreachable))
        }
        medtronicUtil.dismissNotification(MedtronicNotificationType.PumpUnreachable, rxBus)
        ...
        if (tbrCurrent.insulinRate != 0.0 && tbrCurrent.durationMinutes > 0) {
            ...
            Toast.makeText(
                context, info.nightscout.core.ui.R.string.temp_basal_delivery_error, Toast.LENGTH_SHORT
            ).show()
    ...
    protected fun clearTempBasal(): PumpEnactResult {
        ...
        tempBasalMicrobolusOperations.clearOperations()
        result.success = true
        result.comment = rh.gs(string.cancel_temp)
    ...
    private fun scheduleSuspension(
        ...
        return buildPumpEnactResult().enacted(true).success(true).comment(rh.gs(R.string.medtronic_cmd_desc_set_tbr))
    }
    ...
    fun setTempBasalPercent(percent: Int?, durationInMinutes: Int?, profile: Profile?, enforceNew: Boolean): PumpEnactResult {
        ...
        return PumpEnactResult(injector).success(false).enacted(false).comment(
            rh.gs(
                info.nightscout.pump.common.R.string.pump_operation_not_supported_by_pump_driver
            )
        )
    }
    ...
    override fun setTempBasalPercent(
        percent: Int, durationInMinutes: Int,
        profile: Profile, enforceNew: Boolean, func: Function1<PumpEnactResult, *>,
    ): PumpEnactResult {
        ...
        val result: PumpEnactResult = when {
            ...
            percent < 100  -> {
                scheduleSuspension(
                    percent, durationInMinutes, profile, func, 0.0,
                ...
                scheduleTempBasalBolus(
                    percent, durationInMinutes, profile, func,
    ...
    override fun cancelTempBasal(enforceNew: Boolean, callback: Callback?): PumpEnactResult {
        ...
        return PumpEnactResult(injector) //
            .success(false) //
            .enacted(true) //
            .comment(info.nightscout.androidaps.plugins.pump.common.hw.rileylink.R.string.medtronic_pump_status)
    }
    ...
    fun comparePumpBasalProfile(profile: BasalProfile): PumpEnactResult {
        ...
        return if (this.profile == null) {
            ...
            Toast.makeText(
                context,
                rh.gs(R.string.need_manual_profile_set, 40), Toast.LENGTH_LONG
            ).show()
    ...
    private fun handleCalibrationData() {
        ...
        if (lastCalibration != null) {
            val firstAlarmMinutes = sp.getString(rh.gs(R.string.med_link_calibration_first_alarm), "120").toLong()
            val secondAlarmMinutes = sp.getString(rh.gs(R.string.med_link_calibration_second_alarm), "120").toLong()
    ...
    private fun handleChangeStatusData() {
        if (medLinkPumpStatus.pumpRunningState == PumpRunningState.Running) {
            changeStatusTimestamp = System.currentTimeMillis()
            pumpSync.syncStopTemporaryBasalWithPumpIdMedLink(changeStatusTimestamp, changeStatusTimestamp, pumpType, medLinkPumpStatus.serialNumber)
    ...
    @SuppressLint("ResourceType") fun sensorReInit() {
        if (sp.getBoolean(R.bool.key_medlink_handle_cannula_change_event, false)) {
        ...
        if (sens.bgValue.size == 1) {
            if (sens.bgValue.first().sensorUptime != null) {
                if (previousSensorUpTime > sens.bgValue.first().sensorUptime!!) {
                    sensorReInit()
                } else if (previousSensorUpTime <= 0) {
                    //TODO implement read from status command
                }
    ...
    fun glucoseUnit(): BgSync.GlucoseUnit {
        return if (sp.getString(info.nightscout.core.utils.R.string.key_units, Constants.MGDL) == Constants.MGDL) BgSync.GlucoseUnit.MGDL
