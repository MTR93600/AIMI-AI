...
if (AIMI_UAM && AIMI_BreakFastLight && nowdec >= AIMI_BL_StartTime && nowdec <= AIMI_BL_EndTime){
    ...
    console.log("*****Future_sens is not use with light breakfast");
                ...
                console.error("\n");
                console.log("--------------");
                console.log(" 3.1.0.3-dev-e-AIMI-Variant B30-MSSV-100%AIMI 17/12/22 ");
                console.log("--------------");
                if ( meal_data.TDDAIMI3 ){
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && aimi_bg > threshold) { //#MT AIMI
            ...
            if (iTimeActivation && AIMI_BreakFastLight && !profile.temptargetSet && aimi_delta > 0 && aimismb === true && sens_predType == "UAM+" && UAMpredBG >= 220){
            insulinReq = (1 + Math.sqrt(delta)) / 4;
            var microBolus = Math.min(AIMI_UAM_CAP,insulinReq);
            }else if (iTimeActivation && !AIMI_BreakFastLight && !profile.temptargetSet && aimi_delta > 0 && aimismb === true && sens_predType == "UAM+" && UAMpredBG >= 220){
